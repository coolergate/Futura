...
declare global {
	...
	interface PlayerEntityInfo_2 {
		...
		Alive: boolean;
		Id: string;
		CollisionBox: PlayerCollisionModel;
...
function CreatePlayerCollisionBox(Id: string): PlayerCollisionModel {
	...
	const Model = new Instance('Model', PlayersCollisionModels) as PlayerCollisionModel;
	Model.Name = Id;

	CollisionBox.Parent = Model;
	Model.PrimaryPart = CollisionBox;
	return Model;
}
class PlayerEntity extends BaseEntity implements BaseEntityLogic, PlayerEntityInfo_1 {
	...
	constructor() {
		super();
		this.CollisionModel = CreatePlayerCollisionBox(this.Id);
	}
	Spawn(): void {
		this.MaxHealth = 150;
		...
		this.CollisionModel.CollisionBox.CFrame = new CFrame(0, 10000, 0);
	}
	Think(): void {
...
function GenerateInfoFromPlayerController(controller: PlayerEntity): PlayerEntityInfo_1 {
	return {
		...
		CollisionModel: controller.CollisionModel,
	};
}
...
Client_RespawnRequest.OnServerInvoke = player => {
	...
	Controller.CollisionModel.CollisionBox.CFrame = SpawnLocation;
	Controller.CollisionModel.CollisionBox.Anchored = false;
	Controller.CollisionModel.CollisionBox.SetNetworkOwner(player);
...
// * For an module to be considered as an entity constructor
// * It must have 'ent_' at the start of its name

declare global {
	interface BaseEntityModule {
		Spawn(): void;
		Think(): void;
		Death(): void;
		Reset(): void;
